...
<body>
    ...
    <section class="food-menu">
        <div class="container">
            ...
            <?php
                // Write sql query to get all food items from database
                $sql = "SELECT * FROM tbl_food WHERE active='Yes' AND featured='Yes'; ";
                // Execute query
                $res = mysqli_query($conn, $sql);
                // Count rows
                $count = mysqli_num_rows($res);
                // Check whether we have food items in database or not
                if($count > 0) {
                    // There's food. Get data in associative arrays
                    while($row=mysqli_fetch_assoc($res)) {
                        // Get individual data items from individual result rows
                        $id = $row['id'];
                        $title = $row['title'];
                    ...
                    // There's no food items in database
